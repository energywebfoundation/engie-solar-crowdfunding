...
const getInitParams = async (_deployedContract: typeof Contract) => {
  ...
  const minRequiredStake = (process.env.MIN_REQUIRED_STAKE ||
    getEWTAmount('minRequiredStake', ':arrow_heading_up:')) as number;
  return {
    hardCap,
    endDate,
    startDate,
...
const initializeContract = async (_deployedContract: typeof Contract) => {
  const { hardCap, endDate, startDate, signupEnd, signupStart, fullStopDate, minRequiredStake, contributionLimit } =
    await getInitParams(_deployedContract);

  console.log(
    `\n\t\tInit params:

            Signup start = ${new Date(signupStart * 1000)}
            Signup end = ${new Date(signupEnd * 1000)}
            Start Date = ${new Date(startDate * 1000)}
            End Date = ${new Date(endDate * 1000)}
            Full Stop Date = ${new Date(fullStopDate * 1000)}
            HardCap = ${hardCap} wei
...
const init = async (contractAddress: string) => {
  ...
  const contractName = process.env.CONTRACT_NAME || 'StakingTest';
  if (contractAddress) {
    try {
