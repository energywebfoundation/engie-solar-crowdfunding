...
const checkAnswer = (answer: string, promptMode = 'noLoop') => {
  ...
  if (promptMode === 'loop') {
    while (isInvalid(answer)) {
      console.log(
        `\t${emoji.emojify(':rotating_light:')} Invalid option \" ${answer}\" ... Please choose a valid option !`,
      );
...
const getClaimManagerAddress = (hardhatNetwork: string | undefined) => {
  const claimManagerAddress =
    hardhatNetwork === 'ewc' ? process.env.EWC_CLAIM_MANAGER_ADDRESS : process.env.VOLTA_CLAIM_MANAGER_ADDRESS;

  return claimManagerAddress;
};

const getPatronRole = (hardhatNetwork: string | undefined) => {
  const patronRole = hardhatNetwork === 'ewc' ? process.env.PATRON_ROLE_EWC : process.env.PATRON_ROLE_VOLTA;
...
const deployContract = async (contractName: string) => {
  const answer = _prompt(
    `\t${emoji.emojify(':warning:')}\tYou are about to deploy ${contractName} contract to ${
      process.env.HARDHAT_NETWORK
    } network. This will consume some EWT. Do you Confirm ? (Y/n) :  `,
  ) as string;
...
const exportContractAddress = async (contractAdress: string): Promise<string> => {
  return await fs.writeFileSync(
    path.join(__dirname, '..', 'src', 'lib', 'deployedAddress.ts'),
    `export const deployedAddress = '${contractAdress}';`,
  );
};

const deploy = async () => {
  const stakingPoolContract = await deployContract('StakingTest');
